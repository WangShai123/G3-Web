...
if (!$context['valid'] && !$context['success']) {
    wp_redirect(home_url(), 302, 'G3');
    return;
}
get_header();
...
<div class="container">
    <main class="g3-reset-password" style="max-width:520px;margin:64px auto;padding:0 16px;">
        ...
        if ($context['message'] !== '') {
            ...
            echo '<style>.j-tip{margin-bottom:16px;}</style>';
        }
        if ($context['success']) : ?>
            <p>
                <a href="<?php echo esc_url($context['loginUrl']); ?>" class="j-button is-default">
                    <?php _e('Login', 'G3'); ?>
                </a>
            </p>
