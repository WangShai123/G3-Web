...
class WP_Object_Cache {
    ...
    /**
     * 构造
     * 优先检测 Memcached，再检测 Redis，最后回退到内存数组
     */
    public function __construct()
    {
        ...
        if (class_exists('Redis')) {
            try {
                ...
                // 可设置序列化，将复杂值序列化存储
                // 使用 serialize/unserialize 显式处理，避免 phpredis serializer 设置不同步问题
                $this->driver = 'redis';
                return;
            }
            catch (Exception $e) {
    ...
    public function cas($cas_token, $id, $data, $group = 'default', $expire = 0)
    {
        ...
        switch ($this->driver) {
            ...
            //     try {
            //         /**
            //          * md5($curr) → 改成 Redis 内置的 sha1hex()
            //          * 避免 PHP/Redis hash 算法不一致
            //          */
            //         $val = serialize($data);
            //         $lua = <<<LUA
            //         local key     = KEYS[1]
            ...
            //             return 0
            //         end
            //         local currToken = string.lower(redis.sha1hex(curr))
            ...
            //         $res = $this->redis->eval(
            //             $lua,
            //             [$key, (string) $cas_token, $val, (int) $expire],
            //             1
            //         );
    ...
    public function flush()
    {
        ...
        switch ($this->driver) {
            ...
            case 'redis':
                // 注意：flushDB 会清空当前 DB 的所有键，请谨慎使用
