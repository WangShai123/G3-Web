 ...
 * @author Wang Shai
 */
class PremiumListTable extends WP_List_Table {
    ...
    {
        if ($which == "top") {
            echo '<div class="alignleft actions mb-2"><button class="button button-primary add-config">' . __('Add New', 'G3') . '</button></div>';
        }
    }
    ...
    public function process_bulk_actions(): void
    {
        ...
        $roles = Option::get(UserService::PREMIUM_OPTION_KEY);
    ...
    private function initDefaultData(): array
    {
        return [
            'v0' => [
                'name' => __('Enterprise Certification', 'G3'),
                'slug' => 'v0'
            ]
        ];
    }
    ...
    {
        return Option::get(UserService::PREMIUM_OPTION_KEY, $this->default);
    }
    ...
    private function renderAction($item): string
    {
        ...
        return sprintf(
            '<span class="edit-config color-link cursor-pointer" data-name="%s" data-slug="%s">%s</span> <span class="delete-config color-error cursor-pointer" data-slug="%s">%s</span>',
            $item['name'],
            $item['slug'],
            __('Edit'),
            $item['slug'],
