...
class User extends Components {
    ...
    public function resetAvatar($avatar, $id_or_email, $size, $default, $alt): bool|string
    {
        ...
        $defaultAvatar = Context::get(SystemService::OPTION_KEY)['avatar'] ?? '';
    ...
    protected function ajax(): void
    {
        ...
        add_action('wp_ajax_g3_reset_role', function () use ($roles) {
            ...
            if ($slug === 'abandon') {
                ...
                Option::update(UserService::ROLE_OPTION_KEY, $roles);
        ...
        add_action('wp_ajax_g3_edit_custom_role', function () {
            ...
            $roles                = Option::get(UserService::GROUP_OPTION_KEY);
            $roles[$data['slug']] = [
                'name' => $data['name'],
                'slug' => $data['slug']
            ];
            $result               = Option::update(UserService::GROUP_OPTION_KEY, $roles);
            if ($result) {
                Response::ajaxUpdated();
            } else {
                Response::ajaxFailed();
        ...
        add_action('wp_ajax_g3_edit_membership_duration', function () {
            ...
            $array        = Option::get(UserService::DURATION_OPTION_KEY, [], false);
            $array[$slug] = [
                'name'     => $name,
                'slug'     => $slug,
        ...
        add_action('wp_ajax_g3_delete_membership_duration', function () {
            ...
            $array = Option::get(UserService::DURATION_OPTION_KEY, []);
            unset($array[$slug]);
            $result = Option::update(UserService::DURATION_OPTION_KEY, $array);
            if ($result) {
                Response::ajaxDeleted();
        ...
        add_action('wp_ajax_g3_edit_membership_project', function () {
            ...
            $array      = Option::get(UserService::MEMBERSHIP_OPTION_KEY, [], false);
