...
class RouteManifest {
    ...
    private function assertNoConflicts(array $routes): void
    {
        ...
        foreach ($routes as $route) {
            foreach ($this->routeConflictKeys($route) as $key) {
                if (isset($seen[$key])) {
                    $conflicts[] = [
                        'namespace' => $route['namespace'],
                        'route'     => $this->normalizeRoute($route['route']),
                        'method'    => $this->routeMethodFromKey($key),
                        'previous'  => $seen[$key],
                        'current'   => $route,
                    ];
                    continue;
    ...
    private function routeConflictKeys(array $route): array
    {
        ...
        return array_map(static fn(string $method): string => $base . '#' . $method, array_unique($methods));
    }

    private function routeMethodFromKey(string $key): string
    {
        $parts = explode('#', $key);
