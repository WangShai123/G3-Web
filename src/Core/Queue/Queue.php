...
use JEALER\G3\Core\Queue\RedisQueue;
use JEALER\G3\Core\Queue\DatabaseQueue;
...
class Queue {
    ...
    protected static QueueInterface $driver;
    protected static array          $config = [];

    // 实例模式属性
    protected QueueInterface $instanceDriver;
    ...
    protected function deleteCompletedJob(array $job): void
    {
        try {
            ...
            error_log("[G3 Queue] Failed to delete completed job: " . $e->getMessage());
        }
    }
    ...
    public static function unregisterCron(): void
    {
        ...
        if ($checkTimestamp) {
            ...
            error_log("[G3 Queue] Smart cron check unscheduled successfully");
        }
