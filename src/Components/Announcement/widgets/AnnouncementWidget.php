...
class AnnouncementWidget extends WP_Widget {
    ...
    {
        $widget_ops = [
            'classname'                   => 'widget-list announcement-widget',
    ...
    public function widget($args, $instance)
    {
        ...
        foreach ($announcements as $announcement) {
            ...
            echo '<span class="item-index">' . (array_search($announcement, $announcements) + 1) . '</span>';
            echo '<a href="' . get_permalink($announcement['id']) . '" class="item-title">' . $announcement['title'] . '</a></div>';
        }
        echo '</div>';
