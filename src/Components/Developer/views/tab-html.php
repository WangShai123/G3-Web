...
function _renderIconList(array $list)
{
    foreach ($list as $icon) {
        $_ = Image::icon($icon);
        $c = $icon === 'loader' ? ' class="loader"' : '';
        echo "<icon{$c}>{$_}<div>{$icon}</div></icon>";
    }
}
?>
<div class="mt-4">
...
<div>
    ...
    <h4 class="icon-title"><?php _e('Arrow', 'G3'); ?></h4>
    <div class="flex flex-wrap icon-wrap">
        <?php _renderIconList([
            'arrow-up',
            'arrow-down',
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        const { copy, Toast, q, all, listen } = jui;
        for (const item of all('.icon-wrap icon')) {
            listen(item, 'click', () => {
            ...
            listen(item, 'mouseover', () => {
                const svg = q('svg', item);
                if (item.classList.contains('loader')) {
                    svg.classList.add('animate-spin');
                }
                svg.classList.add('color-link');
            })
            listen(item, 'mouseout', () => {
                const svg = q('svg', item);
                svg.classList = '';
            })
        }
