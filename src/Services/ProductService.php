...
class ProductService extends Service {
    ...
    const SKU_CACHE_GROUP   = 'g3_products_sku';
    const SPECS_CACHE_GROUP = 'g3_products_specs';
    /**
    ...
    public function updateSpec(array $data): int|bool
    {
        ...
        if ($id > 0) {
            ...
            if ($result !== false) {
                wp_cache_delete($cacheKey, self::SPECS_CACHE_GROUP);
                return $id;
            }
        } else {
    ...
    {
        $cacheKey = 'all';
        $specs    = wp_cache_get($cacheKey, self::SPECS_CACHE_GROUP);
        ...
        if ($specs === false) {
            ...
            wp_cache_set($cacheKey, $specs, self::SPECS_CACHE_GROUP, DAY_IN_SECONDS);
        }
    ...
    public function deleteSpec(int $id): bool|int
    {
        ...
        if ($result !== false) {
            wp_cache_delete('all', self::SPECS_CACHE_GROUP);
        }
        return $result;
    }
    ...
    public function createSkuCode(int $productId, int $typeId, string $prefix = 'G', string $separator = '-'): string
    {
        ...
        $skuCode = $prefix . "{$separator}" . str_pad($typeId, 2, '0', STR_PAD_LEFT) . "{$separator}" . str_pad($productId, 6, '0', STR_PAD_LEFT);
    ...
    {
        $cacheKey = $productId;
        $skus     = wp_cache_get($cacheKey, self::SKU_CACHE_GROUP);
        ...
        if ($skus === false) {
            ...
            $skus = $rows ?: [];
            wp_cache_set($cacheKey, $skus, self::SKU_CACHE_GROUP, DAY_IN_SECONDS);
        }
    ...
    public function getProduct(int $postId): array|null
    {
        ...
        if ($product === false) {
            $post = $this->postService->init($postId)->cache();
            ...
            wp_cache_set($cacheKey, $product, self::CACHE_GROUP, DAY_IN_SECONDS);
        }
