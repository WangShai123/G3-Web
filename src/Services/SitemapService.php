...
class SitemapService {
    ...
    public function writeStaticFiles(): void
    {
        ...
        $htmlIndex = $this->generateHtmlIndex();
        $xmlIndex  = $this->generateStaticIndexXml();
        System::writeFile($this->sitemapDir . DIRECTORY_SEPARATOR . 'index.html', $htmlIndex);
        System::writeFile($this->sitemapDir . DIRECTORY_SEPARATOR . 'index.xml', $xmlIndex);
        System::writeFile(rtrim(ABSPATH, '/\\') . DIRECTORY_SEPARATOR . 'sitemap.xml', $xmlIndex);
    ...
    private function generateIndex(int $page = 1): string
    {
        return $this->xmlDocument('sitemapindex', $this->indexEntries($page), [
            'local file url: ' . $this->sitemapUrl . '/index.xml',
        ]);
    }
    private function generateStaticIndexXml(): string
    {
        return $this->xmlDocument('sitemapindex', $this->allIndexEntries(), [
            'local file url: ' . $this->sitemapUrl . '/index.xml',
        ]);
    }
