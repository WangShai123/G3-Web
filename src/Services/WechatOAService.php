...
class WechatOAService {
     ...
     * Cache Group
     */
    const CACHE_GROUP = 'g3_wechat_oa';
    ...
    public function processIncomingMessage(mixed $message): bool
    {
        try {
            ...
            $result = false;
            $event  = strtoupper((string) ($messageArray['Event'] ?? ''));
    ...
    {
        $event = strtoupper((string) ($message->Event ?? ''));
    ...
    private static function _insertReply(array $data, array $keywords): int|WP_Error
    {
        ...
        if (!$replyId) {
            return new WP_Error('db_error', 'Failed to insert reply into main table');
        }
    ...
    private static function _updateReply(int $id, array $data, array $keywords): int|WP_Error
    {
        ...
        if ($updated === false) {
            return new WP_Error('db_error', 'Failed to update reply in main table');
        }

        if ($updated === 0) {
            return new WP_Error('not_found', 'Reply not found');
        }
    ...
    public static function deleteReply(array $data): bool|WP_Error
    {
        ...
        if (empty($ids)) {
            return new WP_Error('invalid_ids', 'No valid reply ID(s) provided.');
        }
        ...
        try {
            ...
            if ($deletedReplies === false) {
                throw new Exception('Failed to delete replies from main table.');
            }
            ...
            return new WP_Error('db_error', $e->getMessage() ?: 'Failed to delete reply(s).');
        }
    }
    ...
    {
        if (empty($ids)) {
            return new WP_Error('no_ids', 'No reply IDs provided');
        }
        ...
        if (empty($ids)) {
            return new WP_Error('invalid_ids', 'All provided IDs are invalid');
        }
        ...
        if ($result === false) {
            return new WP_Error('db_error', 'Failed to update reply status');
        }
