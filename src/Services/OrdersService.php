...
class OrdersService {
    ...
    public function createOrderCode(int $source, int $type, int $start = 0, int $baseLength = 8): string|WP_Error
    {
        ...
        if (!$acquired) {
            return new WP_Error(
                'order_lock_failed',
                'Failed to acquire order code lock after multiple attempts.',
    ...
    private function validateOrderData(array $data): bool|WP_Error
    {
        ...
        foreach ($requiredFields as $field => $label) {
            if (!isset($data[$field]) || $data[$field] === '') {
                return new WP_Error(
                    'order_missing_field',
                    sprintf('Missing required field: %s (%s).', $field, $label),
        ...
        foreach ($numericFields as $field) {
            if (!is_numeric($data[$field])) {
                return new WP_Error(
                    'order_invalid_field',
                    sprintf('Invalid field value: %s must be numeric.', $field),
                    ['status' => 400, 'field' => $field]
                );
            }
        }
        ...
        if ((float) $data['final_amount'] > (float) $data['total_amount']) {
            return new WP_Error(
                ...
                'Final amount cannot exceed total amount.',
                ['status' => 400]
            );
        }
    ...
    private function insertOrderItems(int $orderId, array $items): bool|WP_Error
    {
        ...
        foreach ($items as $index => $item) {
            ...
            if (empty($item['product_id']) || empty($item['sku_id']) || !isset($item['unit_price'])) {
                return new WP_Error(
                    'order_item_invalid',
                    sprintf('Invalid item data at index %d.', $index),
            ...
            if ($result === false) {
                return new WP_Error(
                    ...
                    sprintf('Failed to insert order item: %s.', $this->wpdb->last_error),
    ...
    private function insertOrderAddress(int $orderId, array $address): int|WP_Error
    {
        ...
        if ($result === false) {
            return new WP_Error(
                ...
                sprintf('Failed to insert order address: %s.', $this->wpdb->last_error),
                ['status' => 500]
            );
        }
    ...
    public function getOrderItems(int $orderId): array|WP_Error
    {
        ...
        if ($items === null) {
            return new WP_Error(
                ...
                sprintf('Failed to query order items: %s.', $this->wpdb->last_error),
                ['status' => 500]
            );
        }
